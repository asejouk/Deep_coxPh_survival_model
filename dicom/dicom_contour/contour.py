...
def get_slice_HU(image, s):
    image[image == -2000] = 0


    image = (s.RescaleSlope * image.astype(np.float64)).astype(np.int16)
    image += np.int16(s.RescaleIntercept)

    return np.array(image, dtype=np.int16)

def get_resample(image, contour, s, new_spacing=[1, 1, 1]):
    #scan = load_scan(filepath)
    #image = get_image_HU(filepath)


    spacing = np.array([s.SliceThickness, np.array(s.PixelSpacing[0]), np.array(s.PixelSpacing[1])], dtype=np.float32)
    print(spacing)
    ...
    output_image = nd.interpolation.zoom(image, real_resize_factor, mode='nearest')
    output_contour = nd.interpolation.zoom(contour, real_resize_factor, mode='nearest')


    return output_image, output_contour

def get_data(path, index, resampling = False):
    """
    ...
    for k,v in ordered_slices:
        s = dicom.read_file(path + k + '.dcm')
        # get data from contour dict
        if k in contour_dict:
            image = contour_dict[k][0]
            image_HU = get_slice_HU(image, s)
        ...
        # get data from dicom.read_file
            ...
            image_HU = get_slice_HU(img_arr, s)
            images.append(image_HU)
            contours.append(contour_arr)


    output_images = np.array(images)
    output_contours = np.array(contours)


    if resampling:
        # Resample
        print('before resampling image',output_images.shape)
        print('before resampling contour',output_contours.shape)
        output_images, output_contours = get_resample(output_images, output_contours, s, new_spacing=[1, 1, 1])
        print('after resampling image',output_images.shape)
        print('after resampling contour',output_contours.shape)

    return output_images, output_contours
...
def get_data_from_filestore(path, resampling = False):
    """
    Generate image array and contour array
    Inputs:
        path (str): path of the the directory that has DICOM files in it
    ...
    # get slice orders
    ordered_slices, contour_dict = contour_dict_slice_order(path)
    # get contour dict
    #contour_dict = get_contour_dict(path)
...
def coord2pixels_from_filestore(f):
    """
    Given a contour dataset (a DICOM class) and path that has .dcm files of
    corresponding images. This function will return img_arr and contour_arr (2d image and contour pixels)
    Inputs
    ...
    Return
        ...
        contour_arr: 2d np.array of contour with 0 and 1 labels
    """
    #print(f.SeriesDescription)
    #print('Image ID',f.SOPInstanceUID)
    #print('Rescale Intercept',f.RescaleIntercept)
    #print('Rescale Slope',f.RescaleSlope)

    #print('Pixel spacing',f.PixelSpacing)
    #print('Slice Thickness',f.SliceThickness)
    contour_coord = f.ROIContourSequence[0].ContourData
    # x, y, z coordinates of the contour in mm
    coord = []
    for i in range(0, len(contour_coord), 3):
        coord.append((contour_coord[i], contour_coord[i + 1], contour_coord[i + 2]))
    ...
    # extract the image id corresponding to given countour
    # read that dicom file
    img_ID = f.SOPInstanceUID
    #img = dicom.read_file(path + img_ID + '.dcm')
    img_arr = f.pixel_array
    img_arr = img_arr.astype(np.int16)
    # y, x is how it's mapped
    pixel_coords = [(np.ceil(y), np.ceil(x)) for x, y, _ in coord]
...
def contour_dict_slice_order(path):
    """
    Takes path of directory that has the DICOM images and returns
    a ordered list that has ordered filenames
    Inputs
    ...
    for s in os.listdir(path):
        try:
            ...
            # img_arr, contour_arr, img_fname
            contour_list.append(coord2pixels_from_filestore(f))
    ...
    slice_dict = {s.SOPInstanceUID: s.ImagePositionPatient[-1] for s in slices}
    ordered_slices = sorted(slice_dict.items(), key=operator.itemgetter(1))
    return ordered_slices, contour_dict
